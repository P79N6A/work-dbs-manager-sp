...
public class OPOperatorLogDAOImpl implements OPOperatorLogDAO
{
	...
	public int insert(OPOperatorLog obj) throws EduException
	{
		...
		String sql = "INSERT INTO OPOperatorLog(OperLogID,OperID,FunctionID,OperType,TerminalNo,OccurDate,OccurTime,StationAddr,Remark)VALUES(?,?,?,?,?,?,?,?,?)";
		...
		try
		{
			...
			PreparedStmtUtils.setString(pstmt,2,obj.OperID);
			PreparedStmtUtils.setString(pstmt,3,obj.FunctionID);
	...
	public OPOperatorLog find(OPOperatorLog obj) throws EduException
	{
		...
		String sql = "SELECT OperLogID,OperID,FunctionID,OperType,TerminalNo,OccurDate,OccurTime,StationAddr,Remark  FROM OPOperatorLog WHERE OperLogID = ? ";

		log.debug("[find sql]:" + "SELECT OperLogID,OperID,FunctionID,OperType,TerminalNo,OccurDate,OccurTime,StationAddr,Remark  FROM OPOperatorLog WHERE OperLogID = " + obj.OperLogID );
		...
		try
		{
			...
			if(rset.next() == true)
			{
				...
				result.OperID = rset.getString(2);
				result.FunctionID = rset.getString(3);
