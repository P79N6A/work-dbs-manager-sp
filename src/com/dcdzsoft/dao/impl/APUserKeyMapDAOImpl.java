...
public class APUserKeyMapDAOImpl implements APUserKeyMapDAO
{
	private static final Log log = LogFactory.getLog(APUserKeyMapDAOImpl.class.getName());
	...
	public int insert(APUserKeyMap obj) throws EduException
	{
		PreparedStatement pstmt = null;
		int result = 0;

		String sql = "INSERT INTO APUserKeyMap(KeyID,UserID,UserType,UserKey,LastModifyTime,CreateTime,Remark)VALUES(?,?,?,?,?,?,?)";
		...
		try
		{
			...
			PreparedStmtUtils.setString(pstmt,1,obj.KeyID);
			PreparedStmtUtils.setString(pstmt,2,obj.UserID);
			PreparedStmtUtils.setString(pstmt,3,obj.UserType);
			PreparedStmtUtils.setString(pstmt,4,obj.UserKey);
		...
		{
			String errorCode = ErrorCode.ERR_ADDAPUSERKEYMAPFAIL;
	...
	public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "UPDATE APUserKeyMap SET ";
		...
		SQLLog.logUpdate(log,"APUserKeyMap",setCols,whereCols);
		...
		{
			String errorCode = ErrorCode.ERR_MODAPUSERKEYMAPFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(APUserKeyMap obj) throws EduException
	{
		PreparedStatement pstmt = null;
		int result = 0;

		String sql = "DELETE FROM APUserKeyMap WHERE KeyID = ? ";

		log.debug("[delete sql]:" + "DELETE FROM APUserKeyMap WHERE KeyID = " + StringUtils.addQuote(obj.KeyID) );
		...
		{
			String errorCode = ErrorCode.ERR_DELAPUSERKEYMAPFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "DELETE FROM APUserKeyMap WHERE 1=1 " ;
		...
		SQLLog.logDelete(log,"APUserKeyMap",whereCols);
	...
	public boolean isExist(APUserKeyMap obj) throws EduException
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		...
		String sql = "SELECT COUNT(*) FROM APUserKeyMap WHERE KeyID = ? ";

		log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM APUserKeyMap WHERE KeyID = " + StringUtils.addQuote(obj.KeyID) );
		...
		{
			String errorCode = ErrorCode.ERR_QRYAPUSERKEYMAPFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int isExist(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM APUserKeyMap WHERE 1=1 " ;
		...
		SQLLog.logIsExist(log,"APUserKeyMap",whereCols);
	...
	public APUserKeyMap find(APUserKeyMap obj) throws EduException
	{
		APUserKeyMap result = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;

		String sql = "SELECT KeyID,UserID,UserType,UserKey,LastModifyTime,CreateTime,Remark  FROM APUserKeyMap WHERE KeyID = ? ";

		log.debug("[find sql]:" + "SELECT KeyID,UserID,UserType,UserKey,LastModifyTime,CreateTime,Remark  FROM APUserKeyMap WHERE KeyID = " + StringUtils.addQuote(obj.KeyID) );
		...
		try
		{
			...
			if(rset.next() == true)
			{
				result = new APUserKeyMap();
				result.KeyID = rset.getString(1);
				result.UserID = rset.getString(2);
				result.UserType = rset.getString(3);
				result.UserKey = rset.getString(4);
			...
			{
				throw new EduException(ErrorCode.ERR_APUSERKEYMAPNODATA);
			}
		}
		catch(SQLException e)
		{
	...
	public RowSet select(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT * FROM APUserKeyMap WHERE 1=1 ";
	...
	public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT " + fName + " FROM APUserKeyMap WHERE 1=1 ";
