...
public class IMCompanyBoxRightDAOImpl implements IMCompanyBoxRightDAO
{
	private static final Log log = LogFactory.getLog(IMCompanyBoxRightDAOImpl.class.getName());
	...
	public int insert(IMCompanyBoxRight obj) throws EduException
	{
		...
		String sql = "INSERT INTO IMCompanyBoxRight(CompanyID,TerminalNo,BoxNo,Remark)VALUES(?,?,?,?)";
		...
		{
			String errorCode = ErrorCode.ERR_ADDIMCOMPANYBOXRIGHTFAIL;
	...
	public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "UPDATE IMCompanyBoxRight SET ";
		...
		SQLLog.logUpdate(log,"IMCompanyBoxRight",setCols,whereCols);
		...
		{
			String errorCode = ErrorCode.ERR_MODIMCOMPANYBOXRIGHTFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(IMCompanyBoxRight obj) throws EduException
	{
		...
		String sql = "DELETE FROM IMCompanyBoxRight WHERE CompanyID = ?  AND TerminalNo = ?  AND BoxNo = ? ";

		log.debug("[delete sql]:" + "DELETE FROM IMCompanyBoxRight WHERE CompanyID = " + StringUtils.addQuote(obj.CompanyID) +
		...
		{
			String errorCode = ErrorCode.ERR_DELIMCOMPANYBOXRIGHTFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "DELETE FROM IMCompanyBoxRight WHERE 1=1 " ;
		...
		SQLLog.logDelete(log,"IMCompanyBoxRight",whereCols);
	...
	public boolean isExist(IMCompanyBoxRight obj) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM IMCompanyBoxRight WHERE CompanyID = ?  AND TerminalNo = ?  AND BoxNo = ? ";

		log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM IMCompanyBoxRight WHERE CompanyID = " + StringUtils.addQuote(obj.CompanyID) +
						" AND TerminalNo = " + StringUtils.addQuote(obj.TerminalNo) +
						" AND BoxNo = " + StringUtils.addQuote(obj.BoxNo) );
		...
		{
			String errorCode = ErrorCode.ERR_QRYIMCOMPANYBOXRIGHTFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int isExist(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM IMCompanyBoxRight WHERE 1=1 " ;
		...
		SQLLog.logIsExist(log,"IMCompanyBoxRight",whereCols);
	...
	{
		IMCompanyBoxRight result = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;

		String sql = "SELECT CompanyID,TerminalNo,BoxNo,Remark  FROM IMCompanyBoxRight WHERE CompanyID = ?  AND TerminalNo = ?  AND BoxNo = ? ";

		log.debug("[find sql]:" + "SELECT CompanyID,TerminalNo,BoxNo,Remark  FROM IMCompanyBoxRight WHERE CompanyID = " + StringUtils.addQuote(obj.CompanyID) +
						" AND TerminalNo = " + StringUtils.addQuote(obj.TerminalNo) +
						" AND BoxNo = " + StringUtils.addQuote(obj.BoxNo) );
		...
		try
		{
			...
			{
				throw new EduException(ErrorCode.ERR_IMCOMPANYBOXRIGHTNODATA);
			}
		}
		catch(SQLException e)
		{
	...
	public RowSet select(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT * FROM IMCompanyBoxRight WHERE 1=1 ";
	...
	public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT " + fName + " FROM IMCompanyBoxRight WHERE 1=1 ";
