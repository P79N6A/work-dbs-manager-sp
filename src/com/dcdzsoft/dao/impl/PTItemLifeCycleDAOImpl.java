...
public class PTItemLifeCycleDAOImpl implements PTItemLifeCycleDAO
{
	private static final Log log = LogFactory.getLog(PTItemLifeCycleDAOImpl.class.getName());
	...
	public int insert(PTItemLifeCycle obj) throws EduException
	{
		PreparedStatement pstmt = null;
		int result = 0;

		String sql = "INSERT INTO PTItemLifeCycle(WaterID,PackageID,ItemStatus,LastStatus,StatusTime,ZoneID,OperatorID,OperatorType,RecordLevel,LastModifyTime,Remark)VALUES(?,?,?,?,?,?,?,?,?,?,?)";
		...
		try
		{
			...
			PreparedStmtUtils.setString(pstmt,4,obj.LastStatus);
			PreparedStmtUtils.setInt(pstmt,5,obj.StatusTime);
			PreparedStmtUtils.setString(pstmt,6,obj.ZoneID);
			PreparedStmtUtils.setString(pstmt,7,obj.OperatorID);
			PreparedStmtUtils.setString(pstmt,8,obj.OperatorType);
			PreparedStmtUtils.setInt(pstmt,9,obj.RecordLevel);
		...
		{
			String errorCode = ErrorCode.ERR_ADDPTITEMLIFECYCLEFAIL;
	...
	public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "UPDATE PTItemLifeCycle SET ";
		...
		SQLLog.logUpdate(log,"PTItemLifeCycle",setCols,whereCols);
		...
		{
			String errorCode = ErrorCode.ERR_MODPTITEMLIFECYCLEFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(PTItemLifeCycle obj) throws EduException
	{
		PreparedStatement pstmt = null;
		int result = 0;

		String sql = "DELETE FROM PTItemLifeCycle WHERE WaterID = ? ";

		log.debug("[delete sql]:" + "DELETE FROM PTItemLifeCycle WHERE WaterID = " + obj.WaterID );
		...
		{
			String errorCode = ErrorCode.ERR_DELPTITEMLIFECYCLEFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "DELETE FROM PTItemLifeCycle WHERE 1=1 " ;
		...
		SQLLog.logDelete(log,"PTItemLifeCycle",whereCols);
	...
	public boolean isExist(PTItemLifeCycle obj) throws EduException
	{
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		...
		String sql = "SELECT COUNT(*) FROM PTItemLifeCycle WHERE WaterID = ? ";

		log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM PTItemLifeCycle WHERE WaterID = " + obj.WaterID );
		...
		{
			String errorCode = ErrorCode.ERR_QRYPTITEMLIFECYCLEFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int isExist(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM PTItemLifeCycle WHERE 1=1 " ;
		...
		SQLLog.logIsExist(log,"PTItemLifeCycle",whereCols);
	...
	public PTItemLifeCycle find(PTItemLifeCycle obj) throws EduException
	{
		PTItemLifeCycle result = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;

		String sql = "SELECT WaterID,PackageID,ItemStatus,LastStatus,StatusTime,ZoneID,OperatorID,OperatorType,RecordLevel,LastModifyTime,Remark  FROM PTItemLifeCycle WHERE WaterID = ? ";

		log.debug("[find sql]:" + "SELECT WaterID,PackageID,ItemStatus,LastStatus,StatusTime,ZoneID,OperatorID,OperatorType,RecordLevel,LastModifyTime,Remark  FROM PTItemLifeCycle WHERE WaterID = " + obj.WaterID );
		...
		try
		{
			...
			if(rset.next() == true)
			{
				result = new PTItemLifeCycle();
				result.WaterID = rset.getLong(1);
				result.PackageID = rset.getString(2);
				result.ItemStatus = rset.getString(3);
				result.LastStatus = rset.getString(4);
				result.StatusTime = rset.getInt(5);
				result.ZoneID = rset.getString(6);
				result.OperatorID = rset.getString(7);
				result.OperatorType = rset.getString(8);
				result.RecordLevel = rset.getInt(9);
			...
			{
				throw new EduException(ErrorCode.ERR_PTITEMLIFECYCLENODATA);
			}
		}
		catch(SQLException e)
		{
	...
	public RowSet select(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT * FROM PTItemLifeCycle WHERE 1=1 ";
	...
	public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT " + fName + " FROM PTItemLifeCycle WHERE 1=1 ";
