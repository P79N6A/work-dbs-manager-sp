...
public class MBSendReportEmailDAOImpl implements MBSendReportEmailDAO
{
	private static final Log log = LogFactory.getLog(MBSendReportEmailDAOImpl.class.getName());
	...
	public int insert(MBSendReportEmail obj) throws EduException
	{
		...
		String sql = "INSERT INTO MBSendReportEmail(OperID,EmailType,SendAsRights,CreateTime,LastModifyTime,Remark)VALUES(?,?,?,?,?,?)";
		...
		try
		{
			...
			PreparedStmtUtils.setString(pstmt,2,obj.EmailType);
			PreparedStmtUtils.setString(pstmt,3,obj.SendAsRights);
		...
		{
			String errorCode = ErrorCode.ERR_ADDMBSENDREPORTEMAILFAIL;
	...
	public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "UPDATE MBSendReportEmail SET ";
		...
		SQLLog.logUpdate(log,"MBSendReportEmail",setCols,whereCols);
		...
		{
			String errorCode = ErrorCode.ERR_MODMBSENDREPORTEMAILFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(MBSendReportEmail obj) throws EduException
	{
		...
		String sql = "DELETE FROM MBSendReportEmail WHERE OperID = ? ";

		log.debug("[delete sql]:" + "DELETE FROM MBSendReportEmail WHERE OperID = " + StringUtils.addQuote(obj.OperID) );
		...
		{
			String errorCode = ErrorCode.ERR_DELMBSENDREPORTEMAILFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "DELETE FROM MBSendReportEmail WHERE 1=1 " ;
		...
		SQLLog.logDelete(log,"MBSendReportEmail",whereCols);
	...
	public boolean isExist(MBSendReportEmail obj) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM MBSendReportEmail WHERE OperID = ? ";

		log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM MBSendReportEmail WHERE OperID = " + StringUtils.addQuote(obj.OperID) );
		...
		{
			String errorCode = ErrorCode.ERR_QRYMBSENDREPORTEMAILFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int isExist(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM MBSendReportEmail WHERE 1=1 " ;
		...
		SQLLog.logIsExist(log,"MBSendReportEmail",whereCols);
	...
	{
		MBSendReportEmail result = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;

		String sql = "SELECT OperID,EmailType,SendAsRights,CreateTime,LastModifyTime,Remark  FROM MBSendReportEmail WHERE OperID = ? ";

		log.debug("[find sql]:" + "SELECT OperID,EmailType,SendAsRights,CreateTime,LastModifyTime,Remark  FROM MBSendReportEmail WHERE OperID = " + StringUtils.addQuote(obj.OperID) );
		...
		try
		{
			...
			if(rset.next() == true)
			{
				result = new MBSendReportEmail();
				result.OperID = rset.getString(1);
				result.EmailType = rset.getString(2);
				result.SendAsRights = rset.getString(3);
			...
			{
				throw new EduException(ErrorCode.ERR_MBSENDREPORTEMAILNODATA);
			}
		}
		catch(SQLException e)
		{
	...
	public RowSet select(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT * FROM MBSendReportEmail WHERE 1=1 ";
	...
	public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT " + fName + " FROM MBSendReportEmail WHERE 1=1 ";
