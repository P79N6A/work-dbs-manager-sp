...
public class MBTimeLimitDAOImpl implements MBTimeLimitDAO
{
	private static final Log log = LogFactory.getLog(MBTimeLimitDAOImpl.class.getName());
	...
	public int insert(MBTimeLimit obj) throws EduException
	{
		...
		String sql = "INSERT INTO MBTimeLimit(StatusID,StatusName,TimeLimit)VALUES(?,?,?)";
		...
		try
		{
			...
			PreparedStmtUtils.setString(pstmt,1,obj.StatusID);
			PreparedStmtUtils.setString(pstmt,2,obj.StatusName);
			PreparedStmtUtils.setInt(pstmt,3,obj.TimeLimit);
			result = pstmt.executeUpdate();

		}
		catch(SQLException e)
		{
			String errorCode = ErrorCode.ERR_ADDMBTIMELIMITFAIL;
	...
	public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "UPDATE MBTimeLimit SET ";
		...
		SQLLog.logUpdate(log,"MBTimeLimit",setCols,whereCols);
		...
		{
			String errorCode = ErrorCode.ERR_MODMBTIMELIMITFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(MBTimeLimit obj) throws EduException
	{
		...
		String sql = "DELETE FROM MBTimeLimit WHERE StatusID = ? ";

		log.debug("[delete sql]:" + "DELETE FROM MBTimeLimit WHERE StatusID = " + StringUtils.addQuote(obj.StatusID) );
		...
		{
			String errorCode = ErrorCode.ERR_DELMBTIMELIMITFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int delete(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "DELETE FROM MBTimeLimit WHERE 1=1 " ;
		...
		SQLLog.logDelete(log,"MBTimeLimit",whereCols);
	...
	public boolean isExist(MBTimeLimit obj) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM MBTimeLimit WHERE StatusID = ? ";

		log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM MBTimeLimit WHERE StatusID = " + StringUtils.addQuote(obj.StatusID) );
		...
		{
			String errorCode = ErrorCode.ERR_QRYMBTIMELIMITFAIL;
			log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
			throw new EduException(errorCode);
		}
		finally
	...
	public int isExist(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT COUNT(*) FROM MBTimeLimit WHERE 1=1 " ;
		...
		SQLLog.logIsExist(log,"MBTimeLimit",whereCols);
	...
	{
		MBTimeLimit result = null;
		PreparedStatement pstmt = null;
		ResultSet rset = null;

		String sql = "SELECT StatusID,StatusName,TimeLimit  FROM MBTimeLimit WHERE StatusID = ? ";

		log.debug("[find sql]:" + "SELECT StatusID,StatusName,TimeLimit  FROM MBTimeLimit WHERE StatusID = " + StringUtils.addQuote(obj.StatusID) );
		...
		try
		{
			...
			if(rset.next() == true)
			{
				result = new MBTimeLimit();
				result.StatusID = rset.getString(1);
				result.StatusName = rset.getString(2);
				result.TimeLimit = rset.getInt(3);

			}
			else
			{
				throw new EduException(ErrorCode.ERR_MBTIMELIMITNODATA);
			}
		}
		catch(SQLException e)
		{
	...
	public RowSet select(JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT * FROM MBTimeLimit WHERE 1=1 ";
	...
	public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
	{
		...
		String sql = "SELECT " + fName + " FROM MBTimeLimit WHERE 1=1 ";
