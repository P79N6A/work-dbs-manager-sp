...
public class TBServerBoxDAOImpl implements TBServerBoxDAO
{
	...
	public int insert(TBServerBox obj) throws EduException
	{
		...
		String sql = "INSERT INTO TBServerBox(TerminalNo,BoxNo,BoxType,BoxName,DeskNo,DeskBoxNo,BoxStatus,BoxUsedStatus)VALUES(?,?,?,?,?,?,?,?)";
		...
		try
		{
			...
			PreparedStmtUtils.setString(pstmt,8,obj.BoxUsedStatus);
	...
	public TBServerBox find(TBServerBox obj) throws EduException
	{
		...
		String sql = "SELECT TerminalNo,BoxNo,BoxType,BoxName,DeskNo,DeskBoxNo,BoxStatus,BoxUsedStatus  FROM TBServerBox WHERE TerminalNo = ?  AND BoxNo = ? ";

		log.debug("[find sql]:" + "SELECT TerminalNo,BoxNo,BoxType,BoxName,DeskNo,DeskBoxNo,BoxStatus,BoxUsedStatus  FROM TBServerBox WHERE TerminalNo = " + StringUtils.addQuote(obj.TerminalNo) +
						" AND BoxNo = " + StringUtils.addQuote(obj.BoxNo) );
		...
		try
		{
			...
			if(rset.next() == true)
			{
				...
				result.BoxUsedStatus = rset.getString(8);
