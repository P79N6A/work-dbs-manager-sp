...
public class QYStat4SMS extends ActionBean
{
    ...
    public RowSet doBusiness(InParamQYStat4SMS in) throws EduException
    {
        ...
        String sql = "SELECT '" + reportID + "' AS ReportID,OccurYear,OccurMonth,SUM(TotalNum) AS TotalNum,SUM(PwdNum) AS PwdNum,SUM(ExpireNum) AS ExpireNum,SUM(ReminderNum) AS ReminderNum,SUM(DynamicNum) AS DynamicNum,SUM(PickupNum) AS PickupNum,SUM(OtherNum) AS OtherNum FROM V_Stat4SMS a WHERE 1=1 ";
		sql = sql + whereSQL.getPreparedWhereSQL() + limitSQL + "GROUP BY OccurYear,OccurMonth ";//DepartmentID,DepartmentName,DepartmentID,DepartmentName,
