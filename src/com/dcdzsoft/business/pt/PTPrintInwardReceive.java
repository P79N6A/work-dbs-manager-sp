...
/**
 * <p>Title: 自提柜后台运营系统</p>
 * <p>Description: R5报告打印（寄件接收清单报告） </p>
...
public class PTPrintInwardReceive extends ActionBean
{

    public OutParamPrintData doBusiness(InParamPTPrintInwardReceive in) throws EduException
    {
		...
		DMHistoryItemDAO historyPackDAO = daoFactory.getDMHistoryItemDAO();
    	JDBCFieldArray whereCols0 = new JDBCFieldArray();
    	whereCols0.add("LastModifyTime", ">=", DateUtils.addTimeStamp(sysDateTime, -30));//打印30天以内的订单
    	whereCols0.add("ItemStatus", SysDict.ITEM_STATUS_COLLECT_INWARD_RECEIVED);
        ...
        while (RowSetUtils.rowsetNext(rset)) {
        	...
        	if(StringUtils.isEmpty(postmanID)){
        		postmanID = RowSetUtils.getStringValue(rset, "CollectionAgentID");
        	}
        	...
        	if((PrintedFlag&SysDict.MANDATORY_REPORT5_CODE)!=SysDict.MANDATORY_REPORT5_CODE){
        		//修改打印状态
        		PrintedFlag = (PrintedFlag|SysDict.MANDATORY_REPORT5_CODE);
