...
public class PTDeliveryItemQry extends ActionBean {

	public RowSet doBusiness(InParamPTDeliveryItemQry in) throws EduException {
		...
		OPOperOnline operOnline = new OPOperOnline();
		...
		String sql = "SELECT * FROM " + viewName + " a WHERE 1=1 "
				+ whereSQL.getPreparedWhereSQL() + limitSQL;
		java.util.List<String> orderByField = new java.util.LinkedList<String>();
		orderByField.add("LastModifyTime DESC");// CreateTime

		rset = dbSession.executeQuery(sql, orderByField, in.recordBegin,
				in.recordNum, whereSQL);
