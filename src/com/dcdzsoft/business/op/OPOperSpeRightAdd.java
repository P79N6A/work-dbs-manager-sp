...
public class OPOperSpeRightAdd extends ActionBean
{
    ...
    public int doBusiness(InParamOPOperSpeRightAdd in) throws EduException
    {
        ...
        //3.	调用OPOperatorDAO.isExist (管理员编号)查询管理员信息是否存在，如果不存在返回ERR_OPOPERATORNODATA。
		...
		//4.	调用OPOperSpeRightDAO.isExist (特殊权限号, 管理员编号)查询管理员特殊权限设置信息是否存在，如果存在返回ERR_OPOPERSPERIGHTEXISTS。

		//5.	调用UtilDAO.getCurrentDateTime()获得系统日期时间。
		java.sql.Timestamp sysDateTime = utilDAO.getCurrentDateTime();
		java.sql.Date sysDate = DateUtils.toSQLDate(sysDateTime);

        //6.	调用OPOperSpeRightDAO.insert()插入管理员特殊权限设置信息。
