...
public class DBSCommWebSocket {
	...
	public void onError(Session session,Throwable t) 
        ...
        if (root instanceof EOFException) {
            ...
            // ignore it.
        	System.out.println(DateUtils.nowDate() + " " + DateUtils.nowTime() + ",[webSocket onError0] " + t.getMessage());
        }else if (root instanceof IOException) {
            System.out.println(DateUtils.nowDate() + " " + DateUtils.nowTime() + ",[webSocket onError1] " + t.getMessage());
        } else {
            //throw t;
        	//t.printStackTrace();
        	System.out.println(DateUtils.nowDate() + " " + DateUtils.nowTime() + ",[webSocket onError2]= " + t.getClass().getName() + "," + t.getMessage());
        }
	}
	
	@OnMessage
	public void onTextMessage(Session session,String message,boolean last,@PathParam("terminal-no")String terminalNo) {
	...
	public void onBinaryMessage(ByteBuffer bb,@PathParam("terminal-no")String terminalNo){
		//System.out.println("haha:" + byteBufferToString(bb) + "," + terminalNo);
		socketManager.addLastActiveDateTime(terminalNo, new java.util.Date());
	}
